...
//    }
    public ArrayList<NhaCungCap> readDB() {
        ...
        return listSort(dsncc);
    }
    
    
     public static ArrayList<NhaCungCap> listSort(ArrayList<NhaCungCap>list){
        
            Collections.sort(list, new Comparator<NhaCungCap>() {
            @Override
            public int compare(NhaCungCap ob1, NhaCungCap ob2) {
                // Chuyển đổi mã nhân viên thành số và so sánh
                int ma1 = Integer.parseInt(ob1.getMaNCC().replaceAll("\\D", ""));
                int ma2 = Integer.parseInt(ob2.getMaNCC().replaceAll("\\D", ""));
