...
public class QuanLyNhanVienDAO {
    ...
    public ArrayList<NhanVien> readDB() {
        ...
        return listSort(dsnv);
    }
    
      public static ArrayList<NhanVien> listSort(ArrayList<NhanVien>list){
        
            Collections.sort(list, new Comparator<NhanVien>() {
            @Override
            public int compare(NhanVien ob1, NhanVien ob2) {
                // Chuyển đổi mã nhân viên thành số và so sánh
                int ma1 = Integer.parseInt(ob1.getMaNV().replaceAll("\\D", ""));
                int ma2 = Integer.parseInt(ob2.getMaNV().replaceAll("\\D", ""));
