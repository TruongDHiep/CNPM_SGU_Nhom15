...
public class QuanLyKhachHangDAO {
    ...
    public ArrayList<KhachHang> readDB() {
        ...
        return listSort(dskh);
    }
    
    
    public static ArrayList<KhachHang> listSort(ArrayList<KhachHang>list){
        
            Collections.sort(list, new Comparator<KhachHang>() {
            @Override
            public int compare(KhachHang ob1, KhachHang ob2) {
                // Chuyển đổi mã nhân viên thành số và so sánh
                int ma1 = Integer.parseInt(ob1.getMaKH().replaceAll("\\D", ""));
                int ma2 = Integer.parseInt(ob2.getMaKH().replaceAll("\\D", ""));
